...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games AS winner ON winner.winner_id = team_id LEFT JOIN games AS opponent ON opponent.opponent_id = team_id WHERE (winner.year = 2014 AND winner.round = 'Eighth-Final') OR (opponent.year = 2014 AND opponent.round = 'Eighth-Final')")"
